...
pub async fn follow_user(
    ...
    match pool {
        DbPool::Sqlite(p) => {
            sqlx::query("INSERT INTO follows (follower, followee) VALUES ($1, $2) ON CONFLICT DO NOTHING")
                .bind(follower)
                .bind(followee)
                .execute(p)
                .await?;
...
pub async fn is_following(
    ...
    match pool {
        DbPool::Sqlite(p) => {
            let row = sqlx::query("SELECT EXISTS(SELECT 1 FROM follows WHERE follower = $1 AND followee = $2)")
                .bind(follower)
                .bind(followee)
                .fetch_one(p)
                .await?;
