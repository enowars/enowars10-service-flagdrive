...
async def putnoise_4(
    ...
    try:
        await db.set("userdata", (username, password, token, file_id, filename, file_content.decode(), encryption_key))
    except Exception as e:
        logger.error(f"Failed db.set! Error: {e!r}")
        import traceback
        logger.error(traceback.format_exc())
        raise

@checker.getnoise(4)
async def getnoise_4(
    ...
    try:
        username, password, token, file_id, filename, file_content_str, encryption_key = await db.get("userdata")
    except KeyError:
        raise MumbleException("Missing database entry from putnoise")

    file_content = file_content_str.encode()
